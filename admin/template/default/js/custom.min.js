...
$(document).ready(function () {
    ...
    $(add_field).click(function (e) { //on add input button click
        ...
        if (x < max_fields) { //max input box allowed
            x++; //text box increment
            $(wrapper).append('<tr class="">' +
                '<td class="form-group"><input class="form-control" type="text" name="meta[branch' + x + '][branchname]"></td>' +
                '<td class="form-group"><input class="form-control" type="text" name="meta[branch' + x + '][address]"></td>' +
                '<td class="form-group"><input class="form-control" type="text" name="meta[branch' + x + '][url]"></td>' +
                '<td><a href="#" class="remove_field"><i class="fa fa-times"></a></td>' +
                '</tr>'
            ); //add input box
        }
    });
    //adding donation type values
    $(add_field_donation).click(function (e) { //on add input button click
        e.preventDefault();
        if (x < max_fields) { //max input box allowed
            x++; //text box increment
            $(wrapper).append('<tr class="">' +
                '<td class="form-group"><input class="form-control" required type="text" name="donation_type[value][item' + x + '][name]"></td>' +
                '<td class="form-group"><input class="form-control" required type="number" name="donation_type[value][item' + x + '][value]"></td>' +
                '<td><a href="#" class="remove_field"><i class="fa fa-times"></a></td>' +
                '</tr>'
            ); //add input box
        }
    ...
    //changing donation type 
    $(select_option).on("change", function () {
        var type = $(this).val();
        wrapper.empty();
        switch (type) {
            case 'share': donationDetails = '';
                $('.multible-values').show();
                break;
            case 'unit': donationDetails = '';
                $('.multible-values').show();
                break;
            case 'fixed': donationDetails = ' <label class="control-label">القيم الثابتة</label><input required class="form-control" type="number" name="donation_type[value]">';
                $('.multible-values').hide();
                break;
            case 'open': donationDetails = '';
                $('.multible-values').hide();
                break;
            default:
                break;
        }
        $('.donation_type_data').html(donationDetails);
    })
});

//filemanagesr for ck editor
CKEDITOR.replace('ckeditor', {
    filebrowserBrowseUrl: '../helpers/filemanager/dialog.php?type=2&editor=ckeditor&fldr=',
    filebrowserUploadUrl: '../helpers/filemanager/dialog.php?type=2&editor=ckeditor&fldr=',
    filebrowserImageBrowseUrl: '../helpers/filemanager/dialog.php?type=1&editor=ckeditor&fldr='
});
